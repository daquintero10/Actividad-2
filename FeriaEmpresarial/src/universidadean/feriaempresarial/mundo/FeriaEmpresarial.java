...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        ...
        int sumZonC=0;
        for(int i=0; i< empresas.size();i++) {
        	Empresa company = empresas.get(i);	
        	if(company.darTipo().equals("Expositora")) {
        		for(int j=0; j < puestos.length;j++) {
        			String zone= puestos[j].darZona();
        			if(zone.equals("Zona norte")) {
        				sumZonN=sumZonN+ company.darNumeroPersonasAsistentes();
        			}else if(zone.equals("Zona sur")) {
        				sumZonS=sumZonS+ company.darNumeroPersonasAsistentes();
        			}else if(zone.equals("Zona oriente")) {
        				sumZonE=sumZonE+ company.darNumeroPersonasAsistentes();
        			}else if(zone.equals("Zona occidente")) {
        				sumZonO=sumZonO+ company.darNumeroPersonasAsistentes();
        			}else if(zone.equals("Zona centro")) {
        				sumZonC=sumZonC+ company.darNumeroPersonasAsistentes();
        			}
        		}
        	}
        }
        
        if (sumZonN>= sumZonS && sumZonN>= sumZonE && sumZonN>= sumZonO && sumZonN>= sumZonC) {
        	mensaje1="La zona norte es la que tiene mas expositores con " + sumZonN;
        }else if(sumZonS>= sumZonN && sumZonS>= sumZonE && sumZonS>= sumZonO && sumZonS>= sumZonC) {
        	mensaje1="La zona sur es la que tiene mas expositores con " + sumZonS;
        }else if(sumZonE>= sumZonS && sumZonE>= sumZonN && sumZonE>= sumZonO && sumZonE>= sumZonC) {
        	mensaje1="La zona oriente es la que tiene mas expositores con " + sumZonE;
        }else if(sumZonO>= sumZonS && sumZonO>= sumZonE && sumZonO>= sumZonN && sumZonO>= sumZonC) {
        	mensaje1="La zona occidental es la que tiene mas expositores con " + sumZonO;
        }else if(sumZonC>= sumZonS && sumZonC>= sumZonE && sumZonC>= sumZonO && sumZonC>= sumZonN) {
        	mensaje1="La zona central es la que tiene mas expositores con " + sumZonC;
        }
    ...
    public String metodo2() {
        String mensaje2="";
        int max=(MAX_PERSONAS_CENTRO * NUM_PUESTOS_CENTRO)+(MAX_PERSONAS_NORTE * NUM_PUESTOS_NORTE)+(MAX_PERSONAS_SUR * NUM_PUESTOS_SUR) + (MAX_PERSONAS_ORIENTE * NUM_PUESTOS_ORIENTE) + (MAX_PERSONAS_OCCIDENTE * NUM_PUESTOS_OCCIDENTE);
    	int exposit=0;
        
        for(int i =0; i < empresas.size(); i++) {
        	Empresa company = empresas.get(i);
        	String nombreEmpresa= company.darNombre();
        	if(company.darTipo().equals("Expositora")) {
        		for(int j=0; j < puestos.length; j++) {
        			if(puestos[j].estaOcupado() && puestos[j].darNombreEmpresa().equals(nombreEmpresa)) {
        				exposit=exposit+company.darNumeroPersonasAsistentes(); 
        			}
        		}
        	}
        }
        double avg= (exposit*100/max);
        if(avg >= 60) {
        	mensaje2="El punto de equilibro se alcanzó";
        }else {
        	mensaje2="No se alcanzo el punto de equilibrio";
        }
